...
class Solution {
     ...
     func findAllConcatenatedWordsInADict(_ words: [String]) -> [String] {
        ...
        private func wordBreak(_ word: String, _ wordSet: __Set<String>) -> Bool {
            guard word.count > 0 else { return false }
            
            for i in 1..<word.count {
                let prefix = String(Array(word)[0..<i])
                if wordSet.contains(prefix) {
                    let suffix = String(Array(word)[i..<word.count])
                    if wordSet.contains(suffix) || wordBreak(suffix, wordSet) {
                        return true
                    }
                }
            }
    ...
    private class TrieNode {
       ...
       private var root: [Character: TrieNode] = [:]
       //hard to understand
       func findAllConcatenatedWordsInADict2(_ words: [String]) -> [String] {
           root = buildTrie(words)
           var ans: [String] = []
           
           for word in words where isConcatenated([Character](word), 0, root, false) {
               ans.append(word)
           }
       ...
       private func isConcatenated(_ word: [Character], _ i: Int, _ trie: [Character: TrieNode], _ broken: Bool) -> Bool {
           if i == word.count {
               return false
           }
           
           guard let node = trie[word[i]] else { return false }
           
           if i == word.count - 1 {
               return broken && node.isComplete
           }
           
           var ans: Bool = false
           
           if node.isComplete {
               ans = isConcatenated(word, i + 1, root, true)
           }
           
           ans = ans || isConcatenated(word, i + 1, node.children, broken)
           
           return ans
       }
       
       private func buildTrie(_ words: [String]) -> [Character: TrieNode] {
           var ans: [Character: TrieNode] = [:]
           
           for word in words {
               insert([Character](word), &ans)
           }
           
           return ans
       }
       
       private func insert(_ word: [Character], _ trie: inout [Character: TrieNode]) {
           if word.isEmpty {
               return
           }
           
           var node: TrieNode = {
               if let n = trie[word[0]] {
                   return n
               } else {
                   let n: TrieNode = TrieNode(word[0])
                   trie.updateValue(n, forKey: word[0])
                   return n
               }
           }()
           
           for i in 1..<word.count {
               if let n = node.children[word[i]] {
                   node = n
               } else {
                   let n: TrieNode = TrieNode(word[i])
                   node.children.updateValue(n, forKey: word[i])
                   node = n
               }
           }
    ...
    //DP solution is easier but 3 times slower that Trie
    func findAllConcatenatedWordsInADict1(_ words: [String]) -> [String] {
        var result:[String] = []
        var preWords:Set<String> = []
    ...
    func canForm(_ s: String, _ wordDict:Set<String>) -> Bool {
        ...
        for i in 1...charArray.count {
              ...
              for j in stride(from: i, through: 0, by: -1){
                  if dp[j] && wordDict.contains(String(charArray[j..<i])) {
                      dp[i] = true
                      break
                  }
              }
