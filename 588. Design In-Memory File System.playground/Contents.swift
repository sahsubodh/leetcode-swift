...
// check linux find implementation
...
class FileSystem {
    ...
    func getStrings(_ path:String) -> [String] {
        var components = path.components(separatedBy: "/")
        return components.filter{ !$0.isEmpty }
    }
    
    func ls(_ path: String) -> [String] {
        let strings = getStrings(path)
        var cur = root
        for str in strings {
    ...
    func addContentToFile(_ filePath: String, _ content: String) {
        let strings = getStrings(filePath)
        var cur = root
        for str in strings {
            if cur.children[str] == nil {
                cur.children[str] = TrieNode()
